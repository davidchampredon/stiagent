									  ...
									  bool debugInfo)
{
	...
	initFromFile2(file_startpopulation,
				  file_STI_features,
				  file_STI_SFincrease,
				  file_STI_HIVrebound,
				  file_STI_treatment,
...
{
	/// Scan the all individuals in the population
	/// based on their age and disease stage
	/// draws a random variable that will
	/// decide death or not, during the period "prd"
...
//			g << "time,nDeaths,popSize"<<endl;
	...
	for (unsigned long uid=0; uid<_size; uid++)
	{
		...
		{
			double u = uniform01();
			double probaDeath = death_hazard(uid)*prd;
			
			// kill this individual if stochastic event says so
			if (u < probaDeath)
			{
				if (save_trace_file)
				{
...
dcMatrix	Population::getPartnershipsUID()
{
	...
	/*	dcMatrix res(1,2);
	 ...
	 {
	 for(int j=0; j<_PartnerMatrix.getNbCols(); j++)
	 if (_PartnerMatrix(i,j)>0)
	 {
	 unsigned long f_i = _all_UID_femaleUID[i];
	 unsigned long m_j = _all_UID_maleUID[j];
...
dcMatrix	Population::getSpousesUID()
{
	...
	/*
	 int np = _PartnerMatrix.getNbRows();
	 ...
	 // Check if spousal union
	 if (_individual[f_i].isSpouse(m_j))
	 {
	 if (c==0)
	 {
	 res(c,0) = f_i;
	 ...
	 c++; //cout << endl <<  "DEBUG c:" << c;
	 }
	 }
	 }
	 */
	//DEBUG:
	//cout << "*** c = "<<c<< " TOTAL= "<<_totalNumberSpousalPartnerships<<endl;
...
//					}
					...
					/*cout << "Try transm "<< STInameString(sti) <<" ["<<uid_infect<<"->"<<uid_suscep<<"]";
					 cout << " ; IC="<<infectivity<<" ; SF="<<susceptFactor<<" ; SAT[sextype:"<< T<<"]="<<SAT[T];
					 cout << " : nSexActs="<<nSexType[T];
					 cout << " ; maxproba="<<maxProba << " ; OR=" << oddsratio;
					 cout << " ; proba_baseline="<<proba_T<<" ; proba_coinfection="<<proba_T_coinf<<endl;
					 */
				}
			}
