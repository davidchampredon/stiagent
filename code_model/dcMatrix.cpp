...
Matrix::Matrix(string fileName)
{
	...
	for (int i=0; i<line.size(); i++) {		//strlen(line.c_str())
		cout<<i<<"=>"<<line[i]<<endl;
	}
	...
	num_rows = (int) std::count(std::istreambuf_iterator<char>(thefile), std::istreambuf_iterator<char>(), '\n');	
	cout << "numrow="<<num_rows<<endl;
	
	if (num_rows==0)
	{
		thefile.seekg(0,ios::beg);
		num_rows = (int) std::count(std::istreambuf_iterator<char>(thefile), std::istreambuf_iterator<char>(), '\r');	
	}
