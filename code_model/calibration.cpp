...
{
	/// EXPLORING PARAMETERS SPACE
	/// USING LATIN HYPERCUBE SAMPLING
	/// RETURNS A dcDataFrame WITH ALL
	/// INPUT PARAMETERS USED AND THEIR
							  ...
							  int nMonteCarlo, int DHSphase)
{
	...
	//	vector<double> ageGapBreaks			= TAGD.extractColumn(0);
	//	vector<double> ageFirstSexBreaks_f	= TAFS_f.extractColumn(0);
	//	vector<double> ageFirstSexBreaks_m	= TAFS_m.extractColumn(0);
	//	vector<double> ageGapFirstSexSpouseBreaks_f	= TGFSS_f.extractColumn(0);
	//	vector<double> ageGapFirstSexSpouseBreaks_m	= TGFSS_m.extractColumn(0);
	//	vector<double> ageMalesVisitCSWBreaks		= TAMVC.extractColumn(0);
	//
	//	vector<double> lifeSexPrtnrBreaks_f	= TNLS_f.extractColumn(0);
	//	vector<double> lifeSexPrtnrBreaks_m	= TNLS_m.extractColumn(0);
	...
	/*ofstream fStiOut(STI_out.c_str(),ios::app);
	 // headers for STI file
	 for (int c=0;c<S.get_target_STIprevalence_names().size();c++)
	 {
		fStiOut<<S.get_target_STIprevalence_names()[c];
		if (c<S.get_target_STIprevalence_names().size()-1) fStiOut<<",";
		fStiOut<<endl;
	 }
	 stiPrev.WriteToFileCSV(STI_out);
	 fStiOut.close();*/
